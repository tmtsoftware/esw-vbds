...
object VbdsServerTest {
  ...
  def makeQueue(name: String, promise: Promise[ReceivedFile])(implicit system: ActorSystem, mat: Materializer) : SourceQueueWithComplete[ReceivedFile] = {
    Source
  ...
  def makeFile(numBytes: Int, name: String): File = {
    val file = new File(s"$tempDir/$name")
...
//        VbdsServer.start(host, server2HttpPort).await
      ...
      runOn(subscriber1) {
        ...
        val subscribeResponse = client.subscribe(streamName, tempDir, queue).await
...
//        assert(subscribeResponse.status == StatusCodes.OK)
      ...
      runOn(publisher1) {
        ...
        enterBarrier("streamCreated")
        enterBarrier("subscribedToStream")
        val publishResponse = client.publish(streamName, testFile).await
//        publishResponse
        enterBarrier("receivedFile")
      }
